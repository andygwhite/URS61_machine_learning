...
class linearRegression(nn.Module):
    def __init__(self, input_arr, target_arr):
        ...
        input_arr = (pd.get_dummies(pd.DataFrame(input_arr), columns=[0,1,2,4,5]))
        print(input_arr)
    ...
    def trainModel(self):
        ...
        self.__fit(train_dl, 1, self.model, loss_fn, opt)
    ...
    def saveModel(self, path='./first_stage_lr_model_epochs_1.pt'):
...
inputs = data[:, [0, 1, 2, 3, 5, 6]]
targets = data[:, [7,8,9]]

lr_model = linearRegression(inputs, targets)
lr_model.trainModel()
lr_model.saveModel(path='./first_stage_lr_model_epochs_1.pt')
